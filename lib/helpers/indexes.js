...
const getModelTitle = require('./get-model-title');
...
module.exports = class Indexes {
	...
	static async drop(model, currentIndexes) {
		...
		try {
			...
			logger.error(`Error dropping indexes for collection ${getModelTitle(model)}: ${err.message}`);
	...
	static async create(model, currentIndexes) {
		...
		try {
			...
			logger.error(`Error creating indexes for collection ${getModelTitle(model)}: ${err.message}`);
			result = 'collectionFailed';
		}
