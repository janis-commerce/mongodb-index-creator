...
module.exports = class Models {
	...
	static getInstance(ModelClass, client, useReadDB) {
		...
		modelInstance.title = `${modelInstance.databaseKey}.${modelInstance.useReadDB ? 'read' : 'write'}.${ModelClass.table}`;

		if(client) {
			modelInstance.session = new ApiSession({ clientCode: client.code });
			modelInstance.title = `${client.code}.${modelInstance.title}`;
		}
