...
module.exports = model => {
	...
	if(!Array.isArray(model.constructor.indexes)) {
		throw new MongodbIndexCreatorError(`Invalid indexes for collection ${model.table}: indexes must be an array of objects`,
	...
	try {
		...
		throw new MongodbIndexCreatorError(`Invalid indexes for collection ${model.table}: ${err.message}`,
			MongodbIndexCreatorError.codes.INVALID_COLLECTION_INDEXES);
	}
};
