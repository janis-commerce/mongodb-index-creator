...
module.exports = class IndexHelper {
	...
	async drop({ name }) {
		...
		try {
			...
			throw new MongodbIndexCreatorError(`Error dropping index '${name}' for collection ${this.modelTitle}: ${err.message}`, MongodbIndexCreatorError.codes.DROP_INDEX_ERROR)
		}
	...
	async create(index) {
		...
		try {
			...
			throw new MongodbIndexCreatorError(`Error creating index '${index.name}' for collection ${this.modelTitle}: ${err.message}`, MongodbIndexCreatorError.codes.CREATE_INDEX_ERROR);
		}
