...
class MongodbIndexCreator {
	...
	async _createIndexes(config, collections) {
		...
		if(!isObject(config) || !isObject(collections)) {
			throw new MongodbIndexCreatorError('Invalid parameters: Should receive the database config and collections objects.',
				MongodbIndexCreatorError.codes.INVALID_PARAMETERS);
		}
		...
		Object.entries(collections).forEach(([collection, indexes]) => {
			...
			if(typeof collection !== 'string') {
				throw new MongodbIndexCreatorError('Invalid collection name: Should exist and must be a string.',
					MongodbIndexCreatorError.codes.INVALID_COLLECTION_NAME);
			}

			if(!Array.isArray(indexes)) {
				throw new MongodbIndexCreatorError('Invalid indexes: Should exists and must be an array.',
					MongodbIndexCreatorError.codes.INVALID_INDEXES);
			}

			const indexesToCreate = indexes.map(({ name, key, unique }) => {

				if(!isObject(key)) {
					throw new MongodbIndexCreatorError('Invalid index: Should have the key property and must be an object.',
						MongodbIndexCreatorError.codes.INVALID_INDEX);
				}
